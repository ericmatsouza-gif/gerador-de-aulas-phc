...
st.set_page_config(
    page_title="Gerador de Aulas", page_icon="📚", layout="centered"
)
...
class TextRenderer:
  ...
  def __init__(
      self, pdf: FPDF, font_name: str = "DejaVu", base_size: float = 10
  ):
    self.pdf = pdf
...
class PDFMaterial(FPDF):
  ...
  def __init__(self, disciplina: str, ano_escolar: str, assunto: str):
    ...
    if FONT_DIR:
      ...
      self.add_font(
          "DejaVu", style="B", fname=os.path.join(FONT_DIR, "DejaVuSans-Bold.ttf")
      )
      self.add_font(
          "DejaVu",
          style="I",
  ...
  def header(self):
    ...
    if self.page_no() == 1:
      ...
      self.cell(
          0,
          5,
          f"{self.disciplina.upper()} | {self.ano_escolar} | Assunto:"
          f" {self.assunto}",
          align="C",
          new_x=XPos.LMARGIN,
          new_y=YPos.NEXT,
...
def compilar_pdf(
  ...
  for linha_raw in texto_md.split("\n"):
    ...
    if s.startswith("# "):
      ...
      pdf.cell(
          W, 8, f"  {s[2:]}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT
      )
...
Atua em Educação Matemática, Tecnologias Digitais no Ensino, História da Educação Matemática, Políticas Públicas, Educação Ambiental e Esquemas Colaborativos na Educação.
  ...
  st.markdown(
      "💬 **WhatsApp:** [(21) 97048-1891](https://wa.me/5521970481891)"
  )
  st.markdown(
      "✉️ **E-mail:** [ericmatsouza@gmail.com](mailto:ericmatsouza@gmail.com)"
  )

  st.divider()

  st.info(
      "💡 Aplicação para geração de materiais didáticos multidisciplinares."
  )
